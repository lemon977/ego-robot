...
def get_gt_hand_world(
    d_t: dict, side: str, hand_entity_key: str = "hands"
) -> Tuple[Optional[np.ndarray], float]:
    """Extracts ground-truth 4x4 Hand Pose and grasp state."""
    hands = d_t.get("entities", {}).get(hand_entity_key, {})
    if side in hands:
...
def run_teacher_forced_vis(
    ...
    dummy_ds = FlowMatchingDataloader(
        ...
        hand_action_representation=hand_action_representation,
    ...
    hand_entity_key = dummy_ds.hand_entity_key
    ...
    command_dim = 22 if hand_action_representation == "kaihand_joint_state" else 1
    base_dim = (9 + command_dim) * num_hands
    ...
    for i in range(len(frame_dirs)):
        ...
        primary_h, _ = get_gt_hand_world(
            d_t, single_hand_side if single_hand else "left", hand_entity_key
        )
        ...
        for h_idx, side in enumerate(hand_sides):
            ...
            for k in range(1, pred_horizon + 1):
                ...
                if dk:
                    T_h_w_k, _ = get_gt_hand_world(dk, side, hand_entity_key)
            ...
            # PRED Hand Rendering
            T_h_w_0, _ = get_gt_hand_world(d_t, side, hand_entity_key)
