...
class FlowMatchingModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        if hand_action_representation not in {
            ...
            raise ValueError(
                "Unsupported hand_action_representation: "
                f"{hand_action_representation}"
            )
        ...
        self.use_robot_state_conditioning = hand_action_representation != "grasp_binary"

        # --- Action Dimensions (Unified Joint Manifold Router) ---
        self.num_hands = 1 if single_hand else 2
        self.base_action_dim = (9 + self.hand_command_dim) * self.num_hands
        ...
        if self.use_robot_state_conditioning:
            # Per hand: current wrist9 + q + confidence + valid.  These are
            # separate tokens so a missing hand can be masked independently.
            self.robot_state_dim = 9 + self.hand_command_dim + 2
            self.robot_state_proj = nn.Linear(self.robot_state_dim, vision_embed_dim)
            self.robot_state_pos_emb = nn.Parameter(
                torch.randn(1, self.num_hands, vision_embed_dim)
            )
    ...
    def forward(
        ...
        *,
        ...
        x_robot_state: Optional[torch.Tensor] = None,
        robot_state_mask: Optional[torch.Tensor] = None,
        ...
        # 3. Context & Masking
        state_tokens = ict_tokens
        state_mask = ict_mask
        if self.use_robot_state_conditioning:
            if x_robot_state is None or robot_state_mask is None:
                raise ValueError(
                    "Robot action representations require x_robot_state and "
                    "robot_state_mask; silently dropping current q is forbidden"
                )
            expected = (B, self.num_hands, self.robot_state_dim)
            if tuple(x_robot_state.shape) != expected:
                raise ValueError(
            ...
            state_mask = torch.cat([ict_mask, robot_state_mask.bool()], dim=1)
        T_state = state_tokens.size(1)
    ...
    def compute_loss(
        ...
        command_end = nh * (9 + self.hand_command_dim)
        w_dim[..., nh*9 : command_end] = w_command
        ...
        if self.use_robot_state_conditioning and "joint_lower" in targets:
            lower = targets["joint_lower"].float()
            upper = targets["joint_upper"].float()
            expected_limits = (v_pred.shape[0], nh, self.hand_command_dim)
            if tuple(lower.shape) != expected_limits or tuple(upper.shape) != expected_limits:
                raise ValueError(
                    f"joint limits={tuple(lower.shape)}, expected {expected_limits}"
                )
            q_range = (upper - lower).clamp(min=1e-4).reshape(
                v_pred.shape[0], 1, -1
            )
            diff = raw_diff.clone()
        ...
        # Constrain the denoised x1 estimate. The deployment path consumes this
        # direct q output; no retargeter is allowed to repair it afterwards.
        if (
            self.use_robot_state_conditioning
            and lower is not None
            and "action_pred" in preds
            ...
            lambda_velocity = (
                loss_lambdas.get("lambda_velocity", 0.0)
                if loss_lambdas else 0.0
            )
            total_loss = (
                total_loss + lambda_limit * loss_limit
                + lambda_velocity * loss_velocity
            )
            loss_dict["loss_joint_limit"] = loss_limit.detach()
            loss_dict["loss_velocity"] = loss_velocity.detach()
        ...
        if self.use_aux_obj_dynamics:
            pos_masks.append(valid[..., base_D : base_D+3])
            rot_masks.append(valid[..., base_D+3 : base_D+9])
        loss_dict["loss_pos"] = _masked_component(pos_diffs, pos_masks)
        loss_dict["loss_rot"] = _masked_component(rot_diffs, rot_masks)
        loss_dict["loss_g"] = _masked_component(g_diffs, g_masks)
        loss_dict["loss_hand_command"] = loss_dict["loss_g"]
