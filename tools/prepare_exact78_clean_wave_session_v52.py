#!/usr/bin/env python3
"""V5.2.1 successor: prepare one frozen Wave-0 Clean session.

The frozen Wave0 legitimately mixes direct role-mask Grade-B results and the
later zero-denominator reconciliation wrappers.  V3 incorrectly required the
wrapper-only ``pins.immutable_predecessor_result`` field for every row.  This
successor accepts both *explicitly* and records which lineage was consumed;
all referenced bytes and SHA256 values remain fail-closed.
"""
...
def validate(selection_path: Path, session: str) -> tuple[dict, dict]:
    ...
    predecessor_ref = role_result.get("pins", {}).get("immutable_predecessor_result")
    if isinstance(predecessor_ref, dict) and {"path", "bytes", "sha256"} <= set(predecessor_ref):
        predecessor_path = core.checked_ref(predecessor_ref, f"{session}.role_predecessor_result")
        predecessor = core.load_json(predecessor_path)
        role_lineage_mode = "RECONCILED_WRAPPER_TO_IMMUTABLE_PREDECESSOR"
    else:
        # Direct Grade-B role results are already exact-SHA checked above.  They
        # carry config/raw_video directly and are not missing provenance.
        predecessor_path = Path(upstream["role_mask"]["path"]).resolve(strict=True)
        predecessor = role_result
    ...
    return row, {
        ...
        "role_lineage_mode": role_lineage_mode,
        "role_predecessor_result": core.artifact(predecessor_path),
    }
...
def prepare(plan_root: Path, selection: Path, session: str) -> dict:
    ...
    try:
        ...
        receipt = {
            "schema_version": "exact78-clean-wave-session-prepare-v52.1", "created_at": now(),
            ...
            "role_lineage": {
                "mode": context["role_lineage_mode"],
                "effective_predecessor_result": context["role_predecessor_result"],
            },
            "handoff": handoff, "specs": specs,
            "hard_gates": {"all_source_refs_exact": "PASS", "all_frames_and_roles_exact": "PASS",
                           "physical_instance_contract": "PASS", "role_lineage_explicit": "PASS",
                           "object_pixels_excluded_from_removal": "PASS",
