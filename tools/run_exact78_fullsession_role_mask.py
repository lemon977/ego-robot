#!/usr/bin/env python3
"""Full-session SAM3.1 role-removal Mask runner for exact78.
...
def validate(config_path: Path, output: Path, holder: str) -> dict[str, Any]:
    ...
    capture.release()
    if video_count != count or abs(video_fps - float(config["fps"])) > 0.01:
        raise RoleMaskError("video frame/fps mismatch")
    ...
    return {
        ...
        "indices": list(range(count)), "anchor_slot": anchor, "fps": video_fps,
...
def adaptive_gates(validated: dict[str, Any], detailed: dict[str, Any], tracker_meta: dict[str, Any]) -> dict[str, Any]:
    ...
    tracker_fractions = {role: value["visible_coverage_fraction"] for role, value in tracker_meta.items()}
    ...
    gates = {
        ...
        "tracker_expected_visible_coverage_fraction_at_least_0p80": bool(tracker_fractions) and all(value >= 0.80 for value in tracker_fractions.values()),
    ...
    return {"hard_gates": gates, "human_pass_fractions": human_fractions, "tracker_expected_visible_coverage_fractions": tracker_fractions, "tracker_reentry_summary": summary, "details": detailed}
...
def execute(validated: dict[str, Any], output: Path, holder: str) -> int:
    ...
    write(tracker_reentry_manifest, {
        "schema_version": "exact78-fullsession-tracker-reentry-manifest-v2-1",
        "created_at": now(),
        "task": config["task_id"],
    ...
    write(result, {
        "schema_version": "exact78-fullsession-role-mask-result-v1", "created_at": now(),
        ...
        "pins": {"config": ref(validated["config_path"]), "runner": ref(Path(__file__)), "raw_video": ref(validated["raw_video"]), "hawor_npz": ref(validated["hawor_path"]), "gpu_lease_at_execution": ref(lease_snapshot), "checkpoint_sha256": base.CHECKPOINT_SHA256, "sam_build_evidence": build_evidence},
        ...
        "claim_limit": "Full-session bounded-v2.1 role-removal Mask only. Separate task-object identity A/B is mandatory before downstream use.",
    })
