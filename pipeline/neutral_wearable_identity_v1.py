"""Pixel-conservative identity-layer union for archive/legacy/task_cards/32.
...
def identity_union(
    ...
    *,
    side: Side,
    arm_status: str,
    arm_selected_raw_evidence_sha256: str,
    arm_raw_sha256: str,
    ...
    if side not in SIDES or arm_status != "ACCEPT":
        raise WearableIdentityError("arm identity is not accepted for this side")
    if len(arm_selected_raw_evidence_sha256) != 64:
        raise WearableIdentityError("arm selected provenance is incomplete")
    ...
    if mask_sha256(arm_raw_mask) != arm_raw_sha256:
        raise WearableIdentityError("arm provenance SHA mismatch")
